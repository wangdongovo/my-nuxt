<template>
  <div class="container m-auto max-w-screen-xl h-full bg-gray-100">

    <div class="flex justify-evenly pt-5">
      <NuxtLink :to="item.path" class="w-80 h-60 bg-white rounded-xl p-7" v-for="item in titleSource" :key="item.id">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 36 56"
          class="h-14 as-svgicon as-svgicon-delivery as-svgicon-elevated as-svgicon-deliverydelevated" aria-hidden="true"
          role="img">
          <g>
        ...
        <p class="text-2xl font-semibold">{{item.describe}}</p>

      </NuxtLink>
    </div>

    <NuxtLink to="/time">Home page</NuxtLink>
  </div>
</template>
...
const titleSource = ref([
  {id: 1, text: '时间戳', describe: '时间处理相关', path: '/time'},
  {id: 2, text: '时间戳', describe: '时间处理相关', path: '/time'},
  {id: 3, text: '时间戳', describe: '时间处理相关', path: '/time'},
